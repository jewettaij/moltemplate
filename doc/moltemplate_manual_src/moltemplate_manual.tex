...
(These examples are also located in the ``tools/moltemplate/examples'' 
 subdirectory bundled with LAMMPS.)
  %These examples are included with the main moltemplate download.
%(See download instructions below.)
%(which can be downloaded from the official moltemplate web site 
%at \url{http://www.moltemplate.org}). 
...
\subsubsection*{Installation Method 1}
The \textit{``moltemplate.sh''} script and the python scripts it invokes
are located in the \textit{``src''} subdirectory.  If necessary, move the \textit{``moltemplate''} directory to its desired location. (For the sake of this example, let's assume you move it to: \textit{``\$HOME/moltemplate''}.)  You must update your PATH environment variable to include this directory.
%You also need to set your MOLTEMPLATE\_PATH environment variable to 
%point to the \textit{``common''} subdirectory.
...
log back in again for the changes to take effect.}
  ...
  %The ``/'' slash in ``Tubulin/Dimer'' indicates tells moltemplate where to
  %find the definition of the ``Dimer'' molecule 
  %(IE., nested within the definition of ``Tubulin'').
  %The ``/'' character is used to denote both membership 
  %and nesting with in a class or namespace.
  %Once created, you can access the individual protein subunits 
  %in the microtubule using ``/'' syntax.
  %As will be explained later in more detail,
  %to access the type ``A'' subunit of the 77'th tubulin chain, 
  %you would use: ``proteins[77]/a''.
    %(The ``/'' slash character bears similarity to the ``.'' character 
    %in python, or, in some scenarios, the ``.'' and ``::'' characters in C++.)

\subsubsection*{Installation Method 2}
An alternate way to install moltemplate is to simply copy the entire \textit{contents} of moltemplate's ``src'' subdirectory into a directory which is already in your path, such as /usr/local/bin.  (This will require admin privileges.)
...
(See the \textit{SPCE}, \textit{2bead}, and \textit{Butane} 
...
If you want to share atom types between molecules, 
you can place them outside the current molecule definition.
Later you can use file-system-path-like syntax 
(``../'', or ``../../'' or ``/'')
...
(More complex nested relationships are possible.
For example see
  %figure \ref{fig:LPN} and also 
section \ref{sec:paths}.) 
  ...
  % share your molecule definitions (LT files) with others 
  % who may have a different notion of what the ``H'' atom is.)
\textit{(Further details of variable syntax are discussed 
...
However the ``write()'' and ``write\_once()'' commands are not only 
used for generating sections from a DATA file or INPUT scripts. 
...
\subsection{Details/Comments}
Note that it is okay for static and instance variables to share the same 
variable names (such as ``@atom:O'' and ``\$atom:O'', 
or ``@angle:HOH'' and ``\$angle:HOH'').  
These are distinct and are counted separately.
...
Each time a molecule is created using the ``new'' command,
it can be rotated, moved, and scaled.
(Individual atom positions can be customized later
 using the ``write("Data Atoms")'' command. 
 Molecules are free to move during the simulation, of course.)
The initial atomic coordinates 
of a large compound object is typically created 
by moving each individual component.
...
\label{sec:2bead}
As an example, we define a small 2-atom molecule ``2bead'',
construct a short polymer (``peptide'') using it as a building block,
and then create a lattice of peptides.
At each step, we define the relative orientation and position of each unit.

The file below contains the definition of the ``2bead'' molecule. 
It contains coordinates of two atoms,
and their atomic properties (such as their mass, and Lennard Jones parameters).
  %(In this example, the atom IDs are ``\$atom:CA'' and ``\$atom:R'', 
  % and their atom types are ``@atom:CA'' and ``@atom:R''.)
In addition, it defines a bond between the atoms,
as well as parameters for that type of bond (``@bond:sidechain'').
If we were only planning to build a system containing a few isolated 
``2bead'' molecules, then this would be all we need.
  %we don't need this extra information.  
However, later
we intend to connect multiple ``2bead'' molecules 
together later to build a polymer (in this example).  

Consequently, some new sections have been added to the ``2bead'' molecule 
which may seem out of place in a simple 2-atom molecule.
This molecule is slightly more complicated because each ``2bead'' molecule will
eventually be used as a building-block for a larger molecule (``Polymer'').
Consequently, additional information will be used to define additional 
2-body (@bond), 3-body (@angle), and 4-body (@dihedral) interactions 
between atoms in different monomers in the final polymer.
  %We define an additional type of bonded interaction (``@bond:backbone'')
  %as well as several 3-body and 4-body bonded angular interactions.
  %These extra interactions would be unnecessary for a molecule with 
  %only two atoms.
Hopefully the meaning of these interactions will be clear
by the context in which they are used.
(If not, read section \ref{sec:nbody_by_type_intro}.)
...
2bead {
  ...
  # Note: The "..." in "$mol:..." tells moltemplate that this molecule
  #       may be a part of a larger molecule, and to use the larger
  #       parent object’s molecule id number as it’s own, if present.
  ...
  # As usual, force-field parameters ("coeffs") go in the "In Settings" section:
  write_once("In Settings") {
  ...
  write_once("Data Dihedrals By Type") {
    # dihedralType atmType1 atmType2 atmType3 atmType4  bndType1 bnd2   bnd3
    @dihedral:back @atom:CA @atom:CA @atom:CA @atom:CA  @bond:* @bond:* @bond:*
    @dihedral:side @atom:R  @atom:CA @atom:CA @atom:R   @bond:* @bond:* @bond:*
  }

  # Force-field parameters for 4-body (dihedral) interactions:
  write_once("In Settings") {
    #              dihedral-type      K  n   d  w
...
warns moltemplate that the ``2bead'' molecule may be part of a larger molecule.
...
We construct a short polymer by making 7 copies of ``2bead'',
...
Peptide {
  ...
  create_var {$mol}  # optional:force all monomers to share the same molecule-ID
                     # (The "Data Atoms" in 2bead must use the "$mol:..." notation.)

  res1 = new 2bead
  res2 = new 2bead.rot(180.0, 1,0,0).move(3.2,0,0)
  res3 = new 2bead.rot(360.0, 1,0,0).move(6.4,0,0)
  res4 = new 2bead.rot(540.0, 1,0,0).move(9.6,0,0)
  res5 = new 2bead.rot(720.0, 1,0,0).move(12.8,0,0)
  res6 = new 2bead.rot(900.0, 1,0,0).move(16.0,0,0)
  res7 = new 2bead.rot(1080.0, 1,0,0).move(19.2,0,0)
...
\end{verbatim}
The position and orientation of each copy of ``2bead'' 
...
For example, ``res2'' is a copy of ``2bead'' which is first rotated 
...
(Because we are outside the definition of these molecules, 
we must be careful to refer to the atom-IDs and bond-types by their
\textit{full-names}.  Here I'm using the ``@bond:backbone'' settings which
were defined in the ``2bead'' molecule, so I refer to the bond type
as ``@bond:2bead/backbone''.  See section \ref{sec:full_names}.)
  %Recall that a ``@bond:backbone'' variable was 
  %created within the ``2bead'' molecule. 
  %We could define a new bond type, but in this example, 
  %I wanted to use the bond type defined there.)
...
We did this in ``2bead.lt'' using the
\mbox{``write\_once("Angles By Type")''} and 
\mbox{``write\_once("Dihedrals By Type")''} 
commands.
...
You can make 7 copies of the \textit{2bead} molecule this way:
\begin{verbatim}
  res = new 2bead [7]
\end{verbatim}
This creates 7 new \textit{2bead} molecules (named 
\mbox{\textit{res[0]}}, 
\mbox{\textit{res[1]}}, 
\mbox{\textit{res[2]}}, 
\mbox{\textit{res[3]}}, ... 
...
using this command:
\begin{verbatim}
  res = new 2bead [7].rot(180, 1,0,0).move(3.2,0,0)
\end{verbatim}
...
For example:
\begin{verbatim}
  res = new 2bead.scale(1.5) [7].rot(180, 1,0,0).move(3.2,0,0)
\end{verbatim}
In this example, the ``scale(1.5)'' transformation is applied once to 
enlarge every \textit{2bead} monomer initially.
...
composed of of \textit{2bead} and \textit{3bead} monomers
...
\textit{2bead} and \textit{3bead} monomers.
(See figure \ref{fig:random_heteropolymer}.)
\begin{verbatim}
RandPoly50 {
  # Make a chain of randomly chosen monomers:

  monomers = new random([2bead, 3bead], [0.6, 0.4], 123456)
...
\begin{verbatim}
  ...
  #(Note: Both the "2bead" and "3bead" molecules contain atoms
...
%\begin{verbatim}
%  monomers = new random([2bead, 3bead], [30, 20], 123456)
%                 [50].rot(180,1,0,0).move(2.95, 0, 0)
%\end{verbatim}
...
(\mbox{\textit{2bead}} and \mbox{\textit{3bead}} in this example),
...
``2bead'' and 40\% type ``3bead'' (corresponding to 30 and 20, respectively).
However, the resulting polymer (shown in figure \ref{fig:random_heteropolymer})
contains 34 ``2bead'' and 16 ``3bead'' monomers (68\% and 34\%, respectively).
...
\begin{verbatim}
  monomers = new random([2bead, 3bead], [30, 20], 123456)
                 [50].rot(180,1,0,0).move(2.95, 0, 0)
\end{verbatim}
This will create exactly 30 ``2bead'' and 20 ``3bead'' monomers.
...
(``2bead'' and ``3bead''), there is no limit to the number 
of molecule types which appear in these lists (eg ``[2bead, 3bead, 4bead],[0.2,0.3,0.2]'')
...
each molecule's type name in the list (eg ``2bead'' and ``3bead''):
\begin{verbatim}
  monomers = new random([2bead.move(0,0.01,0),
                         3bead.move(0,-0.01,0)], 
                        [30,20],
                        123456)
                 [50].rot(180,1,0,0).move(2.95, 0, 0) 
...
\begin{verbatim}
  monomers[0]    = new 3bead
  monomers[1-48] = new random([2bead, 3bead], [30, 18], 123456)
                       [48].rot(180,1,0,0).move(2.95, 0, 0)
  monomers[49]   = new 3bead
...
of type ``3bead'' 
(while keeping the total number of ``2bead'' and ``3bead'' monomers at 
...
%$atom:dimer/peptides[0]/res[2]/CA $mol:dimer/peptides[1] @atom:2bead/R 0 6.4 8.0 0
\begin{verbatim}
write("Data Atoms") {
  $atom:dimer/peptides[0]/res[2]/CA $mol:dimer/peptides[1] @atom:2bead/R 0 6.4 8.2 0.6
}
\end{verbatim}
Note that because you are outside the definition of the 2bead molecule, 
you must be careful to refer to the atom-ID, molecule-ID, and atom-Type 
variables using their \textit{full} names, as shown above.  (Don't use 
the abbreviated names you normally use. See section \ref{sec:full_names}.)
...
delete dimer/peptides[1]/res[4]/sidechain  #<-- deletes the "sidechain" bond
\end{verbatim}
...
%\begin{verbatim}
%re6s = new 2bead.movecm(0,0,0).rot(180.0, 1,0,0).move(14.2, 0, 0)
%\end{verbatim}
...
%\begin{verbatim}
%res6 = new 2bead.rotcm(180.0, 1,0,0).move(14.2, 0, 0)
%\end{verbatim}
...
Dimer0 {
  write("Data Atoms") {
    $atom:t1 $mol:. @atom:2bead/CA   0.0   23.0   0.0     0.0
    $atom:t2 $mol:. @atom:2bead/CA   0.0   24.7   4.0     0.0
    $atom:t3 $mol:. @atom:2bead/CA   0.0   24.7   8.4     0.0
    $atom:t4 $mol:. @atom:2bead/CA   0.0   23.0   12.4    0.0
  }
  write("Data Bonds") {
    $bond:b1 @bond:2bead/backbone $atom:peptides[0]/res7/CA $atom:t1
    $bond:b2 @bond:2bead/backbone $atom:t1 $atom:t2
    $bond:b3 @bond:2bead/backbone $atom:t2 $atom:t3
    $bond:b4 @bond:2bead/backbone $atom:t3 $atom:t4
    $bond:b5 @bond:2bead/backbone $atom:t4 $atom:peptides[1]/res7/CA
  }
}
...
Again, the ``Angles By Type'' and ``Dihedrals By Type'' sections 
...
%Note: The syntax used in the ``Angles By Type'' and ``Dihedrals By Type''
...
\label{sec:wildcard_bug}
   ...
   \textit{``Angles By Type''}, 
   \textit{``Dihedrals By Type''}, and 
   \textit{``Impropers By Type''}).
...
\mbox{``Angles By Type''}, \mbox{``Dihedrals By Type''}, 
or \mbox{``Impropers By Type''} 
...
write("Data Angles By Type") {
  ...
  @angle:CCCsaturated @atom:C @atom:C @atom:C @bond:sp3 @bond:sp3
}
\end{verbatim}
...
between any 3 atoms of type \mbox{``@atom:C''},
if they are connected by bonds of type \mbox{``@bond:sp3''}.
%\end{list}
