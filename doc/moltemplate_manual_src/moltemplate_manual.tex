...
converted into LT format, and more are planned.  This enables moltemplate
users to build molecules without being forced to manually list all of the
angles, dihedrals, and impropers within every molecule.
Unfortunately, as of 2019-8-26, moltemplate does 
\textit{not} assign partial charges, 
\text{or} infer atom-types from SMILES strings, PDB files,
PSF files or other popular molecular file formats.
In practice, the lack of automatic atom-type determination
can be a huge limitation.
Currently, users must carefully choose the name
of the atom type associated with every atom in the molecule.
For small molecules, they can do this by reading the force-field
file carefully and choosing from among the \@atom types in the list.
This will insure that the force-field selected will understand the atom type
and moltemplate will be able to use it to look up force field information.
For large molecules, they can use an external tool (like AmberTools or PSFGEN)
to generate atom type names and partial charges, and (manually) copy that
information \textit{and convert it, if necessary into a format that is compatible with moltemplate's version of the force-field}.
(Unfortunately, the atom type names in different versions of the
 ...
 As of 2019-8-26, efforts are underway to make this easier.)

Fortunately, external servers such as LibParGen and ATB can also be
used to generate files in LAMMPS or moltemplate format which contain
all of the force-field information.
...
\begin{verbatim}
ltemplify.py -name Mol FILE1.in FILE2.in FILE.data > mol.lt
\end{verbatim}
...
\begin{verbatim}
ltemplify.py -name Mol -atomtype "1 2 3" FILE.in FILE.data > mol.lt
\end{verbatim}
...
\subsection{Inferring atom type names from comments}
\label{sec:ltemplify_atom_type_names}
Officially, the atom types in LAMMPS data files are integers, but some LAMMPS
data files contain additional comments (located in the ``Masses'' section)
which contain the names of each atom type (see below).
When reading a LAMMPS data file ltemplify.py attempts to guess
reasonable names to the types of each atom in your molecules
by reading the comments in ``Masses'' section of the DATA file.
Without such comments, ltemplify.py will generate default atom type names
like ``@atom:type4'' which are not very insightful (and cannot be used to
look up force field parameters).

%The optional arguments
%``\textbf{-infer-comments}'' and ``\textbf{-ignore-comments}''
%control whether ltemplify.py interprets from comments located
%in the ``Masses'' section of the DATA file (if present).
For example if the DATA file you feed to ltemplify.py contains this text:
\begin{verbatim}
Masses
...
4 16.00  # oh
\end{verbatim}
...then ltemplify.py will assign the following @atom-style variables
to these atoms, respectively:
\textit{``@atom:c3''},
\textit{``@atom:h3''},
\textit{``@atom:ho''}, and
\textit{``@atom:oh''}.
However if ``\textbf{-ignore-comments}'' is specified,
(or if comments are absent),
then numeric names will be generated automatically in the usual way
(eg
``\textit{@atom:type1}''
``\textit{@atom:type2}''
``\textit{@atom:type3}'', and
``\textit{@atom:type4}'').

\subsubsection{Nesting atom types within force fields}
Atom type names are frequently used to look up force field information.
Moltemplate allows different molecules in the system to use
different force fields.
Furthermore, different force fields can borrow the same atom type names.
To avoid ambiguity and name clashes, the atom types for common force fields
(like ``OPLSAA'', ``GAFF'', ``COMPASS'')
are usually defined \textit{within the context a force field object}.
(For example, see sections
\ref{sec:nbody_by_type_intro} and \ref{sec:force_field_example_trappe}.)
In this example, these 4 atom types are defined within the ``GAFF'' force field.
In order for moltemplate.sh to be able to locate the force field information
later, you will need to specify which force field they belong to.
You can do this \textit{either} by using the
``\textbf{-name} MOLECULE inherits GAFF''
argument (see section \ref{sec:ltemplify_force_field_examples})
\textit{or} by specifying the \textit{full name} of each atom type,
putting ``GAFF/'' at the beginning of each atom type variable name.
\textit{(See section \ref{sec:full_names} for a discussion of full names.)}
If you decide to do the later, you can provide ltemplify.py with the
``\textbf{-prepend-atom-type} GAFF'' command line argument.
Then the name of each atom type will include the force-field name:
``\textit{@atom:GAFF/c3}'',
``\textit{@atom:GAFF/h3}'',
``\textit{@atom:GAFF/ho}'', and
``\textit{@atom:GAFF/oh}''.
See section \ref{sec:ltemplify_force_field_examples} for examples
of using these two different approaches.
Other force fields, like OPLSAA, or COMPASS (or user-defined force-fields)
can be specified in this way as well.

If you specify the force field you are using, this allows you to
feed ltemplify.py with a DATA file which lacks
\textit{lacks angles, dihedrals, or impropers}.
If your data file contains accurate atom type names, then 
moltemplate.sh can use them to generate these angle, dihedral, and improper
interactions and lookup their force field parameters automatically.
Alternately, if your data file already contains angles, dihedrals or impropers,
then you can use the ``\textbf{-ignore-angles}'' argument to ignore/remove the 
the ``Angles'', ``Dihedrals'', and ``Impropers''
present in your original LAMMPS data file.

Similarly, for many force fields
(including ``OPLSAA'', ``GAFF'', and ``COMPASS''),
it is not necessary to manually specify the type and property of each bond.
(eg, equilibrium rest length, stiffness, etc...).
You only need to specify a list of \textit{which pairs of atoms}
are bonded together.
The bond properties can be determined later by moltemplate.sh
according the atom type names and the force field rules
(eg, contained in ``GAFF'').
To ignore the existing bond type information present
in your data file, you should supply ltemplify.py with the
``\textbf{-ignore-bond-types}''
argument.  This will force ltemplify.py to
ignore the bond types in the (2nd column of the) ``Bonds'' section
of the LAMMPS data file that you provided.
...
in a way which is consistent with the force field you selected.
   ...
   %If the ``\textbf{-ignore-duplicates}'' argument is included,
...
``\textbf{-forbid-duplicates}'' argument.
This will force ltemplify.py to generate an error message whenever
this kind of ambiguity occurs in the atom type names.

\subsubsection{Examples using ltemplify.py with a force field}
\label{sec:ltemplify_force_field_examples}

\subsubsection*{Example 6: Extracting an individual molecule}
...
\begin{verbatim}
# This example creates a new file, "butane.lt", which will contain 
# the instructions for building a "Butane" molecule using "GAFF". First 
# specify which file contains the definition of the "GAFF" force field:

echo "import gaff.lt"  >  butane.lt

# Then use ltemplify.py to extract information from FILE.in, FILE.data

ltemplify.py -name "Butane inherits GAFF" \
             -molid "1" \
             -ignore-angles -ignore-bond-types -forbid-duplicates \
             FILE.in FILE.data >> butane.lt

# Note: if you want to build a simulation containing these molecules,
# you will have to create a "system.lt" file which refers to "butane.lt"
# and then run moltemplate.sh on this file.
\end{verbatim}

This example uses the the comments in ``file.data'' to determine the name 
of each atom type (checking for duplicate errors).
The angle, dihedral, improper, and bond-type information is stripped from 
the original file.data
(and will be generated later according the the rules 
defined in the ``GAFF'' force field).
The name of the molecule (``Butane inherits GAFF'') includes a reference
to the force field (``GAFF'') which will be used to lookup this information.
(Note: The ``GAFF'' force field parameters are typically defined in a file
       named ``gaff.lt''.  Hence in this example we used ``echo''
       to insert a link to ``gaff.lt'' at the beginning of the
       ``butane.lt'' file so that moltemplate.sh will know where to find them.
       Alternatively, this could be done manually by the user.)

\subsubsection*{Example 7: Rebuilding a system using a force field}

The next example reads a description of an entire system from
``FILE.in'' and ``FILE.data''.  Then it ignores the angles, dihedrals,
impropers, and bond-types defined in those files and
regenerates them using the rules defined in the ``GAFF'' force field.

\begin{verbatim}
# This example creates a new file, "system.lt", which will contain 
# the instructions for building a the entire system.  First, specify
# which file contains the definition of the "GAFF" force field:

echo "import gaff.lt"  >  system.lt

# Then use ltemplify.py to extract information from FILE.in, FILE.data

ltemplify.py -prepend-atom-type "GAFF" \
             -ignore-angles -ignore-bond-types -forbid-duplicates \
             FILE.in FILE.data >> system.lt

# Use moltemplate.sh to convert this into LAMMPS format

moltemplate system.lt
\end{verbatim}


\subsection{Automatic generation of bond, angle, dihedral, improper names}

Unfortunately, ltemplify.py currently does not attempt to choose \textit{good}
names for your bonds, or angles.
(And it only makes a feeble attempt to choose reasonable atom type names.
 See section \ref{sec:ltemplify_atom_type_names}.)
...
resulting in bonds and angles with type names like ``@bond:type4'',
``@angle:type7'', and atoms with IDs like ``\$atom:id17538''.
(Atom types are discussed in section \ref{sec:ltemplify_atom_type_names}.)
...
(if the original data file lacks atom type name information).
Similarly you may wish to replace ``@bond:type3'' with ``@bond:C=C'',
``@angle:type11'' with ``@angle:H-C=C'', etc...
