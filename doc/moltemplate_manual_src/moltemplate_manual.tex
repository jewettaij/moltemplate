...
\subsubsection*{Does ``@atom:O'' conflict with ``\$atom:O''?}
\label{sec:vardetails}
No.  It is okay for static and instance variables to share the same 
names.  Moltemplate considers them distinct and they are counted separately.
...
were defined in the ``Monomer'' molecule, so I refer to the bond type
as ``@bond:ForceField/backbone''.  See section \ref{sec:full_names}.)
  %Recall that a ``@bond:Backbone'' variable was 
  %created within the ``Monomer'' molecule. 
