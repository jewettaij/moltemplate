...
converted into LT format.  This enables moltemplate users to
build molecules without being forced to manually list all of the angles, 
dihedrals, and impropers within every molecule.
Unfortunately, as of 2019-8-25, moltemplate does
\textit{not} assign partial charges
\text{or} infer atom-types from SMILES strings, PDB files,
or any other popular molecular file formats.
In practice, this can be a huge limitation.
Instead, currently, users must carefully choose the name
...
information (and convert it) into a format that is compatible with
moltemplate's version of the force-field.
(Unfortunately, the atom type names in different versions of the
 ...
 As of 2019-8-25, efforts are underway to make this easier.)

Fortunately, external servers such as LibParGen and ATB can also be used to
generate files in LAMMPS or moltemplate format which contain all of the
