...
\item \textbf{fix rattle} \ \url{http://lammps.sandia.gov/doc/fix_shake.html}
...
\textit{(For reference, the LAMMPS' ``set'' command is explained here:
        \url{https://lammps.sandia.gov/doc/set.html}.)}
...
\section{Comparing different ways to store force-field information}
\label{sec:spce_example_robust}
The ``.LT'' format is a flexible file format
for storing force field parameters in LAMMPS.
There are a variety of ways to do this.
For completeness, this chapter contains a discussion of these
different methods, and a comparison between them.
However most users are encouraged to follow the conventions
explained in chapters \ref{sec:force_fields} and \ref{sec:inheritance}.



\subsection{Portability: Mixing molecule types}
...
(I did not have to refer to the CH3 atom types as ``@atom:TraPPE/CH3'', or
``@atom:../CH3'' for example.  Eliminating unnecessary redundant text like
``TraPPE/'' and ``../'' reduces the chance of typos or spelling errors.)
