...
I've found that OVITO is very useful for visualizing reactive simulations,
(if you can live with the restriction that the color of bonds between atoms
is determined by those atoms and cannot be set independently).
Using OVITO is pretty straightforward.  It recognizes LAMMPS dump files
ending in ".lammpstrj" automatically.  After loading, click on
"data is a time-series" check box in the lower right.  Particle colors and
sizes can be controlled by clicking on "particle types".
Pseudo-bonds can be drawn between nearby atoms by clicking on
"Add Modificationn", and clicking on "Create Bonds"


---- VMD ----

If you only need to visualize a single snapsot, then VMD can be useful
VMD is not as useful for these kinds of simulations.  However it has some
graphical options that OVITO lacks (such as overlays custom bond colors
and translucent atoms), so I included those instructions below as well:
...
c) Enter:
   ...
   # Typically we would use this command to visualize a DATA file:
   #   topo readlammpsdata system.data full
   # ..however in this example, I generate many different DATA files (with names
   # system_t=10000.data") and I use "atom_style molecular" instead of "full"

   topo readlammpsdata system_t=1000000.data molecular

   # Optional: create a .PSF file (not very useful for reactive simulations)
...
d) Try entering these commands:
    ...
    pbc box -width 1 -shiftcenterrel {-0.5 -0.5 -0.5}
