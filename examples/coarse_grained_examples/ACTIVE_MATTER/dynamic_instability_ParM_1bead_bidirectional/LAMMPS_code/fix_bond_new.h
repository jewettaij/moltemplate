...
FixStyle(bond/new,FixBondNew)

#else

#ifndef LMP_FIX_BOND_NEW_H
#define LMP_FIX_BOND_NEW_H

#include "fix.h"

namespace LAMMPS_NS {

class FixBondNew : public Fix {
 public:
  FixBondNew(class LAMMPS *, int, char **);
  ~FixBondNew();
  int setmask();
  void init();
 ...
 private:
  ...
  int inewtype_override,jnewtype_override;
...
E: Invalid atom type in fix bond/new command

Self-explanatory.

E: Invalid bond type in fix bond/new command

Self-explanatory.

E: Cannot use fix bond/new with non-molecular systems

Only systems with bonds that can be changed can be used.  Atom_style
template does not qualify.

E: Inconsistent iparam/jparam values in fix bond/new command

If itype and jtype are the same, then their maxbond and newtype
settings must also be the same.

E: Fix bond/new cutoff is longer than pairwise cutoff

This is not allowed because bond creation is done using the
pairwise neighbor list.

E: Fix bond/new angle type is invalid

Self-explanatory.

E: Fix bond/new dihedral type is invalid

Self-explanatory.

E: Fix bond/new improper type is invalid

Self-explanatory.

E: Cannot yet use fix bond/new with this improper style

This is a current restriction in LAMMPS.

E: Fix bond/new needs ghost atoms from further away

This is because the fix needs to walk bonds to a certain distance to
acquire needed info, The comm_modify cutoff command can be used to
extend the communication range.

E: New bond exceeded bonds per atom in fix bond/new

See the read_data command for info on setting the "extra bond per
atom" header value to allow for additional bonds to be formed.

E: New bond exceeded special list size in fix bond/new

See the special_bonds extra command for info on how to leave space in
the special bonds list to allow for additional bonds to be formed.

E: Fix bond/new induced too many angles/dihedrals/impropers per atom
...
E: Special list size exceeded in fix bond/new  change/adjacent
