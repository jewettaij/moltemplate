...
Warning:
  ...
  only contains atoms relevant to your problem.
...
for line in f.readlines():
   if "atom" in line and "#" not in line:
      ...
      line=line.split()
      vdw_temp.append([float(line[2]),float(line[3])])
      ...
      line=line.split()
      dihedral.append([int(line[1]),int(line[2]),int(line[3]),
      int(line[4]),float(line[5]),float(line[8]), 
      float(line[11]), 0.0])   	
...
for j,x in enumerate(charge_temp):
  if x[0]==atom[i][0]:
    atom[i][3]=x[1]
    atom[i][4]=vdw_temp[j][0]
    atom[i][5]=vdw_temp[j][1]
    i=i+1
    #print(x[1])
del(charge_temp)
del(vdw_temp)
...
for i,x in enumerate(atom):
  if x[0]<10000:
   g.write("pair_coeff @atom:{0} @atom:{0} lj/cut/coul/cut {1} {2}\n".format(x[0],x[5],x[4]))
...
g.write("dihedral_style  opls\n")
g.write("improper_style  harmonic\n")
g.write("pair_style lj/cut/coul/cut 25.0 25.0\n")
